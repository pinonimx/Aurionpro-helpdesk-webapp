...
import RaiseARequest from './RaiseARequest';
...
function User() {
  ...
  const handleMainPageState = (e) => {
    ...
    else if(e === 'All unassigned issues'){
      setFilteredIssues(sampletabledata.reduce(function (val, key){
  ...
  return (
    <div className="container-new">
      ...
      {mainPageState2 === 'Raise a request' ? <RaiseARequest /> : mainPageState2 === 'Queues' ? <MainHome tabledata={mainPageState.trim() === 'View entire queue' ? sampletabledata : filteredIssues} title={mainPageState} unFilteredData={sampletabledata} pageState={mainPageState2} /> : <Reports />}
    </div>
  );
